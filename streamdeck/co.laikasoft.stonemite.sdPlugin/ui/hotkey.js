(() => {
  ...
  const iconCatalog = window.STONEMITE_LUCIDE_ANIMATED_ICONS || {};
  ...
  function canSaveSettings() {
    return (
      ...
      (draft.targetMode !== "selected" || draft.windowNumbers.length > 0)
    );
  }
  ...
  function updateAutoSaveStatus() {
    ...
    } else if (saveRevision <= savedRevision) {
      saveStatus.textContent = "Changes save automatically.";
    } else if (mappingBusy) {
      saveStatus.textContent = "Checking targets before saving…";
    } else if (!draft.mapping) {
      saveStatus.textContent = "Choose an EQ action to configure this tile.";
    } else if (!canSaveSettings()) {
      saveStatus.textContent = "Choose an action mapped for every target box.";
    }
  }
  ...
  function queueSave(delay = 0) {
    ...
    if (saving || saveRevision <= savedRevision || !canSaveSettings()) return;
  ...
  function markSettingsChanged(delay = 0) {
    saveRevision += 1;
  ...
  async function persistSettings() {
    ...
    try {
      ...
      failed = true;
      formError.textContent = "Stream Deck could not save these changes.";
      formError.hidden = false;
      saveStatus.textContent = "";
    } finally {
      saving = false;
      if (saveRevision > revision) queueSave();
      else if (!failed) saveStatus.textContent = "Saved.";
    }
  }
  ...
  tileLabel.addEventListener("input", () => {
    draft.label = tileLabel.value.trim().slice(0, 14);
    markSettingsChanged(300);
  });
