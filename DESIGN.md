...
- **Error / Disabled:** Invalid fields change the border to error coral, set `aria-invalid`, and expose adjacent role-alert copy. Busy or paired fields are disabled rather than accepting edits that cannot apply.

### Status Panels
- **Style:** The property-inspector connection panel is a two-column status header with a 9px dot, explicit title, explanatory detail, 12px padding, and the panel radius.
...
- **Window-number swap:** The idle key says “NUMBERS / SWAP” and explains the two-step interaction. Armed mode uses the cyan selection rail, says “SWAP / ARMED / PICK CHARACTER,” and temporarily relabels character tiles as “CURRENT” or “SELECT” so the changed action is explicit without relying on color.
...
- **Feedback:** Operation feedback is time-bounded to 1500ms before the current state redraws.
